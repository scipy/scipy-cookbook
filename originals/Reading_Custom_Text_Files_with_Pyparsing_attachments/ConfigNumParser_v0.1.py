...
The main function is _ parseConfigFile _ which allow to read a text file
structured in sections and convert it into a python object with the same
structure as the file but where information is automatically converted into
...
def parseConfigFile(fname):
    ...
    sectionDef = Suppress('[') + keyName + Suppress(']')

    parser = Dict( OneOrMore( 
                Group( 
                    sectionDef + (
                      paramParser()
                    | tableColParser()
